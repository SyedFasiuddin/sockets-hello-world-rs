...
fn echo() {
    ...
    let tcp_listener = match TcpListener::bind("127.0.0.1:8000") {
        ...
        Err(e) => {
            eprintln!("Failed to create a TCP Listener due to: {e}");
            std::process::exit(1);
        }
    };
...
fn socket_file() {
    ...
    let listener = match UnixListener::bind("/tmp/socket-rs-hello.socket") {
        Ok(socket) => socket,
        Err(e) => {
            eprintln!("Failed to create the socket due to: {e}");
            std::process::exit(1);
        }
    };
    ...
    match listener.accept() {
        Ok((mut socket, addr)) => match socket.read(&mut buffer) {
            Ok(size) => print!(
                "Size: {size}\nMsg: {}",
                String::from_utf8(buffer.to_vec()).unwrap()
            ),
            Err(e) => {
                eprintln!("Failed to read from socket (addr: {addr:?}) into buffer due to: {e}")
            }
        },
...
fn chat_client() {
    ...
    match TcpStream::connect("127.0.0.1:8000") {
        Ok(mut stream) => {
            ...
            loop {
                match std::io::stdin().read_line(&mut input) {
                    ...
                    Err(e) => {
                        eprintln!("Failed to read input due to: {e}");
                        std::process::exit(0);
                    }
                }
            }
        ...
        Err(e) => {
            eprintln!("Failed to connect to TCP server due to: {e}");
            std::process::exit(1);
        }
    }
}
...
fn chat_server() {
    ...
    match tcp_listener.accept() {
        ...
        Err(e) => {
            eprintln!("Failed to accept connection due to: {e}");
            std::process::exit(1);
        }
    }
}
